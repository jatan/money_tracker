...
class accountController extends Controller
{
	...
	*/
	public function createAccount_process(){
		...
		if(Request::ajax()) {
			...
			if (array_key_exists('bank', $input) && $input['bank'] == 'cust'){
	...
	*/
	public function syncSingleAccount($id){
		...
		foreach($accounts as $account_key => $account_value ){
			...
			if(isset($bank_account) && $id == $account_value['_id']){
				...
				$bank_account['available_balance'] = isset($account_value['balance']['available']) ? $account_value['balance']['available'] : 0;
				...
				$bank_account['bank_name'] = isset($account_value['institution_type']) ? $account_value['institution_type'] :' ';
				$bank_account ->save();
			}
	...
	private function setTransaction($transaction_value){
		...
		$transaction['location_city'] = isset($transaction_value['meta']['location']['city']) ?$transaction_value['meta']['location']['city'] : ' ';
		$transaction['location_state'] = isset($transaction_value['meta']['location']['state']) ? $transaction_value['meta']['location']['state'] : ' ';

		$transaction['category_id'] = isset($transaction_value['category_id']) ? $transaction_value['category_id'] : 0;
		$transaction['category'] = isset($transaction_value['category']) ? json_encode($transaction_value['category']) : ' ';
